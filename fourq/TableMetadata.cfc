<cfcomponent output="false" hint="This component acts as a generic container for metadata about a database table. It provides utility methods to interact with the metadata and encapsulates the checking for and defalut values of metadata attributes.">
	...
	<cffunction name="init" access="public" output="false" hint="Initializes the instance data for the component" returntype="farcry.farcry_core.fourq.TableMetadata">
	...
	<cffunction name="parseMetadata" access="public" returntype="void" output="false" hint="Parses the given structure and generates metadata that can be used by the DBGateway components." >
		...
		<cfif structKeyExists(arguments.md,'bAbstract') and arguments.bThrowOnAbstract>
			<cfthrow type="farcry.farcry_core.fourq.tablemetadata.abstractTypeException" message="Abstract components cannot be used for table definitions." detail="An attempt was made to generate database table information for an abstract component #md.name# (#md.path#). Abstract components cannot be used for table definitions. Either extend the component or set the bAbstract attribute of the cfcomponent tag to false.">
		</cfif>
	...
	<cffunction name="parseProperty" access="private" output="false" returntype="struct" hint="Parses the data out of cfproperty tag metadata and inserts default values for unspecified attributes">
		...
		<cfif NOT structKeyExists(arguments.data,'type')>
			<cfthrow type="farcry.farcry_core.fourq.TableMetadata.InvalidPropertyException" message="The cfproperty tag for #arguments.data.name# does not have a type attribute." detail="The type attribute of the cfproperty tag is required for the fourq persistence layer." />
		</cfif>
		
		<cfif not listFindNoCase(variables.validDataTypes,arguments.data.type)>
			<cfthrow type="farcry.farcry_core.fourq.TableMetadata.InvalidPropertyException" message="The cfproperty tag for #arguments.data.name# has an invalid value for the type attribute." detail="The list of valid datatypes is #variables.validDataTypes#. The type attribute has a value of ""#arguments.data.type#""." />
		</cfif>
		...
		<cfif structKeyExists(arguments.data,'arrayProps')>
			...
			<cfloop from="1" to="#arrayLen(arrayProps)#" index="i">
			  ...
			  <cfif listFindNoCase(reservedFieldNames,fieldName)>
			    <cfthrow type="farcry.farcry_core.fourq.TableMetadata.InvalidArrayPropertyException" message="The cfproperty tag for #arguments.data.name# is using a reserved field name in the arrayProps attribute." detail="The list of reserved field names is #reservedFieldNames#. The arrayProps attribute has a value of ""#arguments.data.arrayProps#""." />
			  </cfif>
			  
			  <cfif not listFindNoCase(variables.validDataTypes,dataType)>
					<cfthrow type="farcry.farcry_core.fourq.TableMetadata.InvalidArrayPropertyException" message="The cfproperty tag for #arguments.data.name# has an invalid data type in the arrayProps attribute." detail="The list of valid datatypes is #variables.validDataTypes#. The arrayProps attribute has a value of ""#arguments.data.arrayProps#""." />
				</cfif>
