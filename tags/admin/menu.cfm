...
<cfelse>
	...
	<form id="subjump" action="" method="get" class="iframe-nav-form">
		<select name="sub" onchange="location=this.value;return false;"><cfloop from="1" to="#arraylen(asubsections)#" index="i">
		<cfif request.dmsec.oAuthorisation.fCheckXMLPermission(asubsections[i].xmlAttributes)><cfoutput>
			<option value="#cgi.script_path#?sub=#asubsections[i].xmlAttributes.id#"<cfif subsectionid eq asubsections[i].xmlAttributes.id> selected="selected"</cfif>>#asubsections[i].xmlAttributes.label#</option></cfoutput></cfif></cfloop>
		</select>
	</form>
	</cfoutput>
	<cfelse>
