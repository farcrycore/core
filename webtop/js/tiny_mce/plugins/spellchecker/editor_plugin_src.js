...
var TinyMCE_SpellCheckerPlugin = {
		...
		if ((e.type == "click" || e.type == "contextmenu") && elm) {
			do {
				if (tinyMCE.getAttrib(elm, 'class') == "mceItemHiddenSpellWord") {
					...
					self._sendAjax(self.baseURL + "/Spellcheck.cfm", self._ajaxResponse, 'post', args);
		...
		switch (command) {
			case "mceSpellCheck":
				if (!inst.spellcheckerOn) {
					...
					// Setup arguments
					args += 'id=' + inst.editorId + "|" + (++self._counter);
					args += '&cmd=spell&check=' + encodeURIComponent(self._getWordList(inst.getBody())).replace(/%20/g, '+');
					args += '&lang=' + escape(inst.spellCheckerLang);
		...
		switch (cmd) {
			case "spell":
				if (xml.documentElement.firstChild) {					
					...
					if(tinyMCE.getParam('spellchecker_ReportMispellings', false)) {
	...
	_getWordList : function(n) {
		...
		for (i=0; i<nl.length; i++) 
			if(nl[i].parentNode.nodeName != 'STYLE' && nl[i].parentNode.nodeName != 'SCRIPT') nv += nl[i].nodeValue + " "; 
