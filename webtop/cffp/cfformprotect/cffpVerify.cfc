...
<cfimport taglib="/farcry/core/tags/webskin" prefix="skin" />
	...
	<cffunction name="init" access="public" output="true" returntype="cffpVerify">
		...
		<cfargument name="stConfig" required="false" default="#structNew()#" type="struct" />
		
		<cfset var configItem = "" />
		...
		<cfif not structIsEmpty(arguments.stConfig)>
			<cfloop collection="#arguments.stConfig#" item="configItem">
				<cfset variables.Config["#configItem#"] = arguments.stConfig["#configItem#"] />
			</cfloop>
		</cfif>
		...
		<cfif AkismetKeyIsValid>
			...
			<cftry>
				...
				<cfhttp url="http://#getConfig().akismetAPIKey#.rest.akismet.com/1.1/comment-check" timeout="10" method="post">
					...
					<cfif Len(getConfig().akismetFormEmailField) and structKeyExists(arguments.FormStruct, "#getConfig().akismetFormEmailField#")>
						<cfhttpparam name="comment_author_email" type="formfield" value="#arguments.FormStruct[getConfig().akismetFormEmailField]#" />
					</cfif>
					<cfif Len(getConfig().akismetFormURLField) and structKeyExists(arguments.FormStruct, "#getConfig().akismetFormURLField#")>
						<cfhttpparam name="comment_author_url" type="formfield" value="#arguments.FormStruct[getConfig().akismetFormURLField]#" />
					</cfif>
					<cfif Len(getConfig().akismetFormBodyField) and structKeyExists(arguments.FormStruct, "#getConfig().akismetFormBodyField#")>
	...
	<cffunction name="logFailure" acces="private" output="false" returntype="void">
		...
		<cfif StructKeyExists(arguments.TestResults,"mouseMovement") AND NOT arguments.TestResults.mouseMovement.Pass>
			<cfset LogText = LogText & "<br />--- No mouse movement was detected." />
		</cfif>
					
		<cfif StructKeyExists(arguments.TestResults,"usedKeyboard") AND NOT arguments.TestResults.usedKeyboard.Pass>
			<cfset LogText = LogText & "<br />--- No keyboard activity was detected." />
		</cfif>
					
		<cfif StructKeyExists(arguments.TestResults,"timedFormSubmission") AND NOT arguments.TestResults.timedFormSubmission.Pass>
			<cfif isDefined("formTimeElapsed")>
				<cfset LogText = LogText & "<br />--- The time it took to fill out the form was " />
				<cfif formTimeElapsed lt getConfig().timedFormMinSeconds>
			...
			<cfelse>
				<cfset LogText = LogText & "<br />--- The time it took to fill out the form did not fall within your
								configured threshold of #getConfig().timedFormMinSeconds#-#getConfig().timedFormMaxSeconds#
								seconds." /><!---   Also, I think the form data for this field was tampered with by the spammer. --->
			</cfif>
		</cfif>
					
		<cfif StructKeyExists(arguments.TestResults,"hiddenFormField") AND NOT arguments.TestResults.hiddenFormField.Pass>
			<cfset LogText = LogText & "<br />--- The hidden form field that is supposed to be blank contained data." />
		</cfif>
		
		<cfif StructKeyExists(arguments.TestResults,"SpamStrings") AND NOT arguments.TestResults.SpamStrings.Pass>
			<cfset LogText = LogText & "<br />--- One of the configured spam strings was found in the form submission." />
		</cfif>
		...
		<cfif StructKeyExists(TestResults,"TooManyUrls") AND NOT arguments.TestResults.tooManyUrls.Pass>
		      <cfset LogText = LogText & "<br />--- There were too many URLs in the form contents." />
		</cfif>
		...
		<cflog file="#application.ApplicationName#-#arguments.LogFile#" text="#LogText#" />
