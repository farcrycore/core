...
<cfif len(url.objectId)>
	...
	<cfoutput>
	<script>
		window.location="#application.url.farcry#/navajo/edit.cfm?objectId=#NewObjID#&type=#stProps.typename#<cfif structKeyExists(url, "finishUrl")>&finishUrl=#url.finishUrl#</cfif><cfif structKeyExists(url, "iframe")>&iframe=#url.iframe#</cfif>";
	</script>
	</cfoutput>
</cfif>
