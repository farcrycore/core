...
<ft:form bAddFormCSS="false" class="clearfix" bFocusFirstField="true" action="#application.fapi.fixURL(removeValues='mfacancel')#">
	...
	<cfoutput>
		<fieldset>
			...
			#stFields.code.html#
			<cfif bHasEmail><p class="help-block"><cfif bEmailSent><admin:resource key="security.mfa.challenge.emailsent">We've emailed you a code.</admin:resource> </cfif><ft:button rendertype="button" class="btn-link" value="mfaEmailSend" validate="false" rbkey="security.mfa.buttons.emailsend" text="#bEmailSent ? 'Resend code' : 'Email me a code'#" /></p></cfif>
