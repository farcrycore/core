...
<cfif structkeyexists(stObj,"versionID") and len(stObj.versionID)>
	<cfset stLocal.qArchives = application.fapi.getContentObjects(typename="dmArchive",lProperties="objectid,datetimecreated,createdby,objectWDDX",archiveID_eq=stObj.versionID,orderBy="datetimecreated desc") />
<cfelse>
	<cfset stLocal.qArchives = application.fapi.getContentObjects(typename="dmArchive",lProperties="objectid,datetimecreated,createdby,objectWDDX",archiveID_eq=stObj.objectid,orderBy="datetimecreated desc") />
</cfif>
...
<cfloop query="stLocal.qArchives">
	...
	<cfwddx action="wddx2cfml" input="#stLocal.qArchives.objectWDDX#" output="stLocal.stTemp" />
	<cfset stProfile = oProfile.getProfile(username=stLocal.stTemp.lastupdatedby) />
	<cfif structkeyexists(stProfile,"lastname") and len(stProfile.lastname)>
...
<cfelseif structkeyexists(stObj,"status") and stObj.status eq "approved">
...
<cfelseif not structkeyexists(stObj,"status")>
	...
	<cfset stProfile = oProfile.getProfile(username=stLocal.stApproved.lastupdatedby) />
	<cfif structkeyexists(stProfile,"lastname") and len(stProfile.lastname)>
		<cfset querysetcell(stLocal.qVersions,"label","Live - #stProfile.firstname# #stProfile.lastname#") />
	<cfelse>
		<cfset querysetcell(stLocal.qVersions,"label","Live - #listfirst(qLog.userid[1],'_')#") />
	</cfif>
</cfif>
