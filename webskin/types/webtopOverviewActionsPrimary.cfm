...
<!--- <ft:form>
	...
	<extjs:item title="#application.rb.getResource('workflow.headings.mainactions@text','Main Actions')#"> --->
			...
			<cfswitch expression="#stobj.status#">
				<cfcase value="draft"> <!--- DRAFT STATUS --->
					...
					<cfif stOverviewParams.stPermissions.iEdit EQ 1>
						...
						<ft:button icon="#application.url.webtop#/images/crystal/32x32/actions/edit.png" iconPos="top" value="Edit this content item" rbkey="workflow.buttons.edit" url="edittabEdit.cfm?objectid=#stobj.objectid#&ref=#url.ref#&typename=#stobj.typeName#" />
					</cfif>
					...
					<cfif stOverviewParams.stPermissions.iApprove eq 1 OR stOverviewParams.stPermissions.iApproveOwn EQ 1>
						<cfif stobj.objectid NEQ stOverviewParams.objectid_previousversion>
							<ft:button value="Send content item live" rbkey="workflow.buttons.sendlive" url="#application.url.farcry#/navajo/approve.cfm?objectid=#stobj.objectid#&status=approved" />		
						<cfelse>
							<ft:button value="Approve this item yourself" rbkey="workflow.buttons.approveyourself" url="#application.url.farcry#/navajo/approve.cfm?objectid=#stobj.objectid#&status=approved" />
						</cfif>
					</cfif>
				...
				<cfcase value="approved">	
					...
					<cfif bHasDraft EQ 0 AND stOverviewParams.stPermissions.iEdit eq 1 AND bDraftVersionAllowed>
						<ft:button value="Create an editable draft version" rbkey="workflow.buttons.createdraft" url="#application.url.farcry#/navajo/createDraftObject.cfm?objectID=#stobj.objectID#&typename=#stobj.typeName#&ref=#url.ref#" />
					</cfif>
					<cfif stOverviewParams.stPermissions.iApprove eq 1 OR stOverviewParams.stPermissions.iApproveOwn EQ 1>
						<cfset buttonValue = application.rb.getResource("sendBackToDraft") />
						<cfif structKeyExists(stobj,"versionID") AND bHasDraft>
							<ft:button value="Send this content item back to draft (deleting the draft version)" rbkey="workflow.buttons.sendbacktodraftdeletedraft" url="#application.url.farcry#/navajo/approve.cfm?objectid=#stobj.objectid#&status=draft&typename=#stobj.typeName#&ref=#url.ref#" />
						<cfelse>
							<ft:button value="Send this content item back to draft" rbkey="workflow.buttons.sendbacktodraft" url="#application.url.farcry#/navajo/approve.cfm?objectid=#stobj.objectid#&status=draft&typename=#stobj.typeName#&ref=#url.ref#" />
						</cfif>
					</cfif>
				</cfcase>
			</cfswitch>
		<cfelse>	<!--- content items without a status --->
			<!--- check user can edit --->
			<cfif stOverviewParams.stPermissions.iEdit EQ 1>
				<ft:button size="large" color="green" icon="#application.url.webtop#/images/crystal/32x32/actions/edit.png" iconPos="top" value="Edit this content item" rbkey="workflow.buttons.edit" url="edittabEdit.cfm?objectid=#stobj.objectid#&ref=#url.ref#&typename=#stobj.typeName#" />
			</cfif>
		</cfif>
		...
		<ft:button value="Preview" rbkey="workflow.buttons.preview" url="#application.url.webroot#/index.cfm?objectid=#stobj.objectid#&flushcache=1&showdraft=1" target="#target#" />
<!--- 		
	</extjs:item>
			...
			<cfswitch expression="#stobj.status#">
				<cfcase value="draft"> <!--- DRAFT STATUS --->
					...
					<cfif stOverviewParams.stPermissions.iEdit EQ 1>
						<!--- MJB: added url.ref so that the edit methods know they were initially called by the overview page and they can return here if they so desire. --->
						<cfif stobj.objectid NEQ stOverviewParams.objectid_previousversion>
							<ft:button value="Restore live content item over this draft" rbkey="workflow.buttons.restorelivecontent" onClick="confirmRestore('#stobj.parentid#','#stobj.objectid#');" />
						</cfif>
					</cfif>
					...
					<cfif stOverviewParams.stPermissions.iRequest eq 1>
						<cfif stobj.objectid NEQ stOverviewParams.objectid_previousversion>
							<ft:button value="Request approval" rbkey="workflow.buttons.requestapproval" url="#application.url.farcry#/navajo/approve.cfm?objectid=#stobj.objectid#&status=requestapproval&ref=#url.ref#" />
						<cfelse>
							<ft:button value="Request approval for this content item" rbkey="workflow.buttons.requestapprovalforcontent" url="#application.url.farcry#/navajo/approve.cfm?objectid=#stobj.objectid#&status=requestapproval&ref=#url.ref#" />	
						</cfif>
					</cfif>
							...
							<cfif stobj.objectid EQ stOverviewParams.objectid_previousversion>
								...
								<!--- check user can delete --->
									<cfif stOverviewParams.stPermissions.iDelete eq 1>
										...
										<ft:button value="Delete" rbkey="workflow.buttons.delete" url="navajo/delete.cfm?ObjectId=#stobj.objectId#&#returnto#&ref=#url.ref#" confirmText="Are you sure you wish to delete this content item?" />
									</cfif>
									...
									<cfif stOverviewParams.stPermissions.iTreeSendToTrash eq 1 and stobj.typeName eq "dmNavigation">
										<ft:button value="Send to trash" rbkey="workflow.buttons.sendtotrash" url="navajo/move.cfm?srcObjectId=#stobj.objectId#&destobjId=#application.navid.rubbish#&ref=#url.ref#" confirmText="Are you sure you wish to trash this item?" />
									</cfif>
								</cfif>
							<cfelse>
								<ft:button value="Delete this draft version" rbkey="workflow.buttons.deletedraft" url="edittabEdit.cfm?objectid=#stOverviewParams.objectid_previousversion#&deleteDraftObjectID=#stobj.ObjectID#&typename=#stobj.typeName#&ref=#url.ref#" confirmText="Are you sure you wish to delete this content item?" />
							</cfif>
					</cfif>
				</cfcase>
				...
				<cfcase value="pending"> <!--- PENDING STATUS --->
					...
					<cfif stOverviewParams.stPermissions.iEdit EQ 1 AND stobj.bAlwaysShowEdit EQ 1>
						<cfif stobj.objectid NEQ stOverviewParams.objectid_previousversion>
							<ft:button value="Restore live content item over this draft" rbkey="workflow.buttons.restorelive" url="" onclick="confirmRestore('#stobj.parentid#','#stobj.objectid#')" />
						</cfif>
					</cfif>
					
					<cfif stOverviewParams.stPermissions.iApprove eq 1> <!--- check user can approve object --->
						<ft:button value="Send content item live" rbkey="workflow.buttons.sendlive" url="#application.url.farcry#/navajo/approve.cfm?objectid=#stobj.objectid#&status=approved&ref=#url.ref#" />
						<!--- send back to draft --->
						<ft:button value="Send this content item back to draft" rbkey="workflow.buttons.sendbacktodraft" url="#application.url.farcry#/navajo/approve.cfm?objectid=#stobj.objectid#&status=draft&ref=#url.ref#" />
					</cfif>
				</cfcase>
				...
				<cfcase value="approved">	
					...
					<cfif stOverviewParams.stPermissions.iEdit EQ 1 AND stobj.bAlwaysShowEdit EQ 1>
						<cfif stobj.objectid NEQ stOverviewParams.objectid_previousversion>
							<ft:button value="Restore live content item over this draft" rbkey="workflow.buttons.restorelivecontent" url="" onclick="confirmRestore('#stobj.parentid#','#stobj.objectid#');" />
						</cfif>
					</cfif>
					...
					<cfif listContains(application.navid.home,stobj.objectid) EQ 0 AND listContains(application.navid.root,stobj.objectid) eq 0>
						<!--- check user can delete --->
						<cfif stOverviewParams.stPermissions.iDelete eq 1>
							<ft:button value="Delete" rbkey="workflow.buttons.delete" url="navajo/delete.cfm?ObjectId=#stobj.objectId#&typename=#stobj.typeName#&ref=#url.ref#" confirmText="Are you sure you wish to delete this content item?" />
						</cfif>
	...
	<cfif stobj.typename EQ  "dmNavigation">
		<cfif StructKeyExists(stOverviewParams.stPermissions,"iCreate") and stOverviewParams.stPermissions.iCreate eq 1>
			...
			<cfif ArrayLen(aTypesUseInTree)>
				<cfoutput>
				<select id="createContent" name="createContent">
