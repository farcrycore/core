...
<cfset stArgs.stMetadata.ftJQueryOnEdit = "window.editingtree = this; var catiframe = $j('<iframe width=\'100%\' height=\'700px\' src=\'#application.url.farcry#/conjuror/invocation.cfm?objectid='+node.id+'&typename=dmCategory&method=edit&ref=refresh&module=\' scrolling=\'auto\' frameborder=\'0\'></iframe>').appendTo($j('##cateditframe').html('')); var loaded = setInterval(function(){ if (catiframe[0].contentWindow.document.readyState !== 'complete') return false; clearInterval(loaded); catiframe[0].contentWindow.$j(catiframe[0].contentDocument).delegate('form','submit',function(){ $j('.farcry-main').block(); }); },50);" />
<cfset stArgs.stMetadata.ftJQueryOnAdd = "window.editingtree = this; var catiframe = $j('<iframe width=\'100%\' height=\'700px\' src=\'#application.url.farcry#/conjuror/invocation.cfm?objectid='+newid+'&typename=dmCategory&method=edit&ref=refresh&module=\' scrolling=\'auto\' frameborder=\'0\'></iframe>').appendTo($j('##cateditframe').html('')); var loaded = setInterval(function(){ if (catiframe[0].contentWindow.document.readyState !== 'complete') return false; clearInterval(loaded); catiframe[0].contentWindow.$j(catiframe[0].contentDocument).delegate('form','submit',function(){ $j('.farcry-main').block(); }); },50);" />
...
<cfset stArgs.stMetadata.ftJQueryOnUpdateError = "if (code === 'treechanged') $j('##bubbles').prepend('<div class=\'alert alert-error\'>'+message+' <a href=\'##\' class=\'refresh-tree\'>Refresh tree?</a></div>'); else $j('##bubbles').prepend('<div class=\'error\'>'+message+'</div>');" />
