...
function _ltrim(string){
	...
	while( whitespace.indexOf(string.charAt(0)) != -1) && string.length != 0 ) string = string.substring(1);
	return string;
}
...
function _rtrim(string){
	...
	while( whitespace.indexOf(string.charAt(string.length-1)) != -1) && string.length != 0 ) string.substring(0,string.length-1);
	return string;
}
...
function _listSum(string,delim){
	...
	iValue = 0;
	for( var i=0; i < tmp.length; i++ ) iValue += parseInt(tmp[i]);
	return iValue;
}
