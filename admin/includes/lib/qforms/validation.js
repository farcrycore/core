...
function _Field_isPassword(field, minlen, maxlen){
	var minlen = _param(arguments[1], 1);   // default minimum length of password
	var maxlen = _param(arguments[2], 255); // default maximum length of password
...
function _Field_isDifferent(field){
	if( this.compare(field) ){
		this.error = "The " + this.description + " and " + field.description + " must be different.";
	}
}
...
function _Field_isDate(mask){
	...
	if( sYear == null ){
		...
		if( lst30dayMonths.indexOf("," + iMonth + ",") > -1 ){
			...
			if( (iDay == 29) && (iYear%4 != 0 ) ) this.error = "An invalid day was provided for the " + this.description + " field.";
			else if( (iDay < 1) || (iDay > 28 ) ) this.error = "An invalid day was provided for the " + this.description + " field.";
		} else {
...
function _Field_isPhoneNumber(len){
	var len = parseInt(_param(arguments[0], 10));
