...
|| VERSION CONTROL ||
$Header: /cvs/farcry/core/admin/navajo/container_rules.cfm,v 1.11 2005/10/27 05:09:23 guy Exp $
$Author: guy $
...
<cfelse> <!--- all good show form --->
	...
	<label for="label"><b>#application.adminBundle[session.dmProfile.locale].title#</b>
		<input type="textbox" name="label" id="label" value="#stobj.label#"><br />
	</label><cfelse>
	<input type="hidden" name="label" value="#stobj.label#"></cfif>

	<label for="source"><b>#application.adminBundle[session.dmProfile.locale].availableRuleTypes#:</b>
	...
	<option value="#qActiveRules.objectid#">#application.rules[qActiveRules.typename].displayname#</option></cfloop>
	</select><br />
		<span class="f-toolwrap">
		<!-- TODO: i18n -->
		...
		<input type="button" value="#application.adminBundle[session.dmProfile.locale].deleteRule#" onClick="deleteRule(this.form.dest);" onmouseover="this.className='f-delete f-deletehover'" onmouseout="this.className='f-delete'" class="f-delete" style="padding-right:65px" />					 
		</span>
	</label>
	<div class="f-submit-wrap">
	<input type="Submit" name="Update" value="#application.adminBundle[session.dmProfile.locale].commitChanges#" class="f-submit">
	</div>
</cfif>
