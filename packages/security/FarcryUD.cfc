<cfcomponent displayname="FarCry User Directory" hint="Provides the interface for the FarCry user directory. Extends MFAUserDirectory, which contributes the multi-factor engine; this component supplies the credential store, the legacy migration suite, and the MFA account hooks." extends="MFAUserDirectory" output="false"
	...
	<!--- =============================
	  ...
	  Implementations of the seam MFAUserDirectory calls back into: the only
	  MFA methods that reach into farUser/farGroup. The engine itself lives on
	  the abstract MFAUserDirectory base.
	============================== --->
	...
	<cffunction name="resetLoginFailures" access="private" output="false" returntype="void" hint="HOOK impl: clears the shared login-failure counter after a successful second factor">
		<cfargument name="userKey" type="string" required="true" />
		<cfset application.fapi.getContentType("farUser").resetLoginFailures(objectid=arguments.userKey) />
	</cffunction>

	<cffunction name="getUseridForKey" access="private" output="false" returntype="string" hint="HOOK impl: resolves a user key to the login userid for event logging; empty string when not found">
		<cfargument name="userKey" type="string" required="true" />
