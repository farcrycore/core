...
|| VERSION CONTROL ||
$Header: /cvs/farcry/farcry_core/ui/Attic/index.cfm,v 1.10 2003/07/17 04:39:16 brendan Exp $
$Author: brendan $
$Date: 2003/07/17 04:39:16 $
$Name: b131 $
$Revision: 1.10 $
...
<span class="description" onclick="location.href='index.cfm'" onmouseover="style.cursor='hand'" title="#application.config.general.siteTitle# | Home">#application.config.general.siteTagLine#</span>
		...
		<cfscript>
			...
			iDynamicTab = oAuthorisation.checkPermission(reference="policyGroup",permissionName="MainNavDynamicTab");
		...
		<cfoutput><div class="mainTabArea" align="right"></cfoutput>
			...
			<admin:tabs>
				...
				<cfif iDynamicTab eq 1>
					<cfif url.section eq "dynamic">
						<admin:tabitem class="activetab" href="#application.url.farcry#/index.cfm?section=Dynamic" target="_top" title="News Articles" text="Dynamic">
					<cfelse>
						<admin:tabitem class="tab" href="#application.url.farcry#/index.cfm?section=Dynamic" target="_top" title="News Articles" text="Dynamic">
					</cfif>
				</cfif>
				<cfif iAdminTab eq 1>
				...
				<admin:tabs>
					...
					<cfswitch expression="#url.section#">
						...
						<cfdefaultcase>
							...
							<!--- Get Root Level --->
							<cfinvoke 
							 component="#application.packagepath#.farcry.tree" method="getRootNode" returnvariable="getRootNodeRet">
								<cfinvokeargument name="dsn" value="#application.dsn#"/>
								<cfinvokeargument name="typename" value="dmNavigation"/>
							</cfinvoke>
							
							<!--- Get Level 1 Nodes --->
							<cfinvoke component="#application.packagepath#.farcry.tree" method="getChildren" returnvariable="getChildrenRet">
								<cfinvokeargument name="dsn" value="#application.dsn#"/>
							...
							<cfloop query="getChildrenRet">
								...
								<cfif currentrow eq 1>
									<admin:tabitem class="activesubtab" href="navajo/overview_frame.cfm?rootobjectid=#stobj.objectid#" target="treeFrame" text="#stobj.title#" onclick="synchTab('treeFrame','activesubtab','subtab','#stobj.objectid#')" id="#stobj.objectid#">
								<cfelse>
						...
						<cfcase value="dynamic">
							<!--- permission checks --->
							<cfscript>
								iDynamicCategorisationTab = oAuthorisation.checkPermission(reference="policyGroup",permissionName="DyamicCategorisationTab");
							</cfscript>
														
							<cfif iDynamicTab eq 1>	
								<admin:tabitem class="activesubtab" href="dynamic/dynamicMenuFrame.cfm?type=general" target="treeFrame" text="News" onclick="synchTab('treeFrame','activesubtab','subtab','DynamicTab')" id="DynamicTab">
								<admin:tabitem class="subtab" href="dynamic/dynamicMenuFrame.cfm?type=xml" target="treeFrame" text="XML Export" onclick="synchTab('treeFrame','activesubtab','subtab','XMLTab')" id="XMLTab">
							</cfif>
							<cfif iDynamicCategorisationTab eq 1>
						...
						<cfcase value="admin">
							...
							<cfif iAdminGeneralTab eq 1>
								<admin:tabitem class="subtab" href="admin/adminMenuFrame.cfm?type=Stats" target="treeFrame" text="Statistics" onclick="synchTab('treeFrame','activesubtab','subtab','AdminStatsTab')" id="AdminStatsTab">
							</cfif>
						</cfcase>
							...
							<cfloop from="1" to="#ArrayLen(tabElements)#" index="i">
								<cfif structKeyExists(tabElements[i].xmlattributes,"permission")>
									...
									<cfif iCustomTab eq 1>
										<admin:tabitem class="#IIF(i EQ URL.subtabindex ,DE('activesubtab'),DE('subtab'))#" href="admin/customadminMenuframe.cfm?subtabindex=#i#&parenttabindex=#URL.PARENTTABINDEX#" target="treeFrame" text="#tabElements[i].xmltext#" onclick="synchTab('treeFrame','activesubtab','subtab','AdminGeneralTab#i#')" id="AdminGeneralTab#i#">
									</cfif>	
								</cfif>
							</cfloop>
						...
						<cfcase value="security">
							<!--- permission checks --->						
							<cfscript>
								iSecurityAuditTab = oAuthorisation.checkPermission(reference="policyGroup",permissionName="SecurityAuditTab");
							...
							<cfif iSecurityAuditTab eq 1>
								<admin:tabitem class="subtab" href="security/securityMenuFrame.cfm?type=audit" target="treeFrame" text="Audit" onclick="synchTab('treeFrame','activesubtab','subtab','auditTab')" id="auditTab">
							</cfif>
						</cfcase>
					</cfswitch>
				</admin:tabs>
			...
			<div class="tabMain">
				<div class="tabTitle" id="title"></cfoutput>		
					...
					<cfif url.section eq "customadmin">
						<cfoutput>#application.customAdminXML.customtabs.parenttab[url.parenttabindex].xmltext#&nbsp;</cfoutput>
					<cfelse>
					...
					<cfif url.section eq "Site">
						...
						<form name="zoom" style="display:inline;">
							...
							<cfif key neq "root">
								<cfoutput><option value="navajo/overview_frame.cfm?rootobjectid=#application.navid[key]#">#key#</option></cfoutput>
							</cfif>
						...
						<span style="position: absolute; top:8px; right:30px;"><a href="##" onClick="javascript:window.open('legend.cfm','','width=350,height=500,scrollbars=yes,left=200,top=5');"><img src="images/legend.jpg" alt="Legend" border="0"></a></span>
				...
				<div class="tabIframeWrapper"></cfoutput>
					...
					<cfswitch expression="#url.section#">
						...
						<cfcase value="customadmin">
							<cfset defaultPage="admin/customadminMenuFrame.cfm?parenttabindex=#url.parenttabindex#">
						</cfcase>
						<cfcase value="security">
		  ...
		  <div class="tabMain">
			...
			<div class="tabIframeWrapper"></cfoutput>
				...
				<cfswitch expression="#url.section#">
					...
					<cfcase value="stats">
						<cfset defaultPageRight="stats/statsHome.cfm">
					</cfcase>
				</cfswitch>
