...
|| VERSION CONTROL ||
$Header: /cvs/farcry/farcry_core/ui/admin/Attic/coapiTypes.cfm,v 1.3 2003/07/17 01:51:10 brendan Exp $
$Author: brendan $
	...
	switch(form.action){
		case "deleteproperty":
		 ...
		 case "renameproperty":
		 {
		  ...
		  case "repairproperty":
		 {
		 	alterType.repairProperty(typename=form.typename,srcColumn=form.property,srcColumnType=alterType.getDataType(application.types[form.typename].stProps[form.property].metadata.type));
			alterType.refreshCFCAppData(typename=form.typename);
			break;
		 }
		 default:
